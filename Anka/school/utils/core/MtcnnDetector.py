...
class MtcnnDetector(object):
    ...
    def generate_bbox(self, map, reg, scale, threshold):
        ...
        dx1, dy1, dx2, dy2 = [reg[0, i, t_index[0], t_index[1]]
                              for i in range(4)]
            ...
            while min(current_height, current_width) > net_size:
                ...
                for current_y in current_y_list:
                    for current_x in current_x_list:
                        cropped_im = im_resized[:, :, current_y:current_y +
                                                net_size, current_x:current_x + net_size]
...
face candidates:%d, current batch_size:%d"%(num_boxes, batch_size)
            ...
            while min(current_height, current_width) > net_size:
                ...
                boxes = self.generate_bbox(
                    cls_map[0, 1, :, :], reg, current_scale, self.thresh[0])
